...
export default function UserFormModal({ isOpen, onClose, onSuccess, user }: UserFormModalProps) {
  const [formData, setFormData] = useState<UserCreate | UserUpdate>({
    ...
    usuario: '',
    password: '',
    roleId: 'role_admin', // por defecto Administrador (role_admin existe)
    activo: true
  })
  ...
  const validate = (): boolean => {
    ...
    else if (!user && formData.password && formData.password.length < 4) newErrors.password = 'Mínimo 4 caracteres'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (user) {
        ...
        await usersService.create(formData as UserCreate)
      }
      onSuccess()
      onClose()
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="glass-card rounded-2xl w-full max-w-md p-6 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-white/80 text-sm mb-1">{user ? 'Contraseña (dejar en blanco para no cambiar)' : 'Contraseña *'}</label>
          ...
          <div className="flex justify-end space-x-3 pt-2">
            <button type="button" onClick={onClose} className="px-4 py-2 rounded-full bg-white/10 text-white hover:bg-white/20 transition">Cancelar</button>
            <button type="submit" disabled={loading} className="px-4 py-2 rounded-full bg-white/20 text-white font-semibold hover:bg-white/30 transition disabled:opacity-50">
